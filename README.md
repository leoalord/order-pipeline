...
In-flight Place Order requests are capped at `API_ACCEPT_CONCURRENCY` (32, a loose door fuse). Extra requests get a counted HTTP `429` (`door busy`) and create **no** order row and no intake key. That is not kitchen fullness (sim `429` `kitchen busy` / `courier busy` — the order already exists, the worker retries the same key) and not stock (a later permanent fail). Place and cancel never call the sims. If calibrate later 429s at the door before kitchen 3×, raise the knob — do not treat door 429 as the rush beat.

A quiet 1-item chips order walks `placed` → `confirmed` → `being_prepared` → `ready` → `out_for_delivery` → `delivered`. Kitchen cook is 12s plus rail wait when pans are full (20 pans cook at once; a quiet kitchen has wait 0). A quiet near trip is 12s. First kitchen poll waits until a pan is free (`service_started_at`), then `being_prepared` is its own GET-visible commit before `ready`. Dispatch hangs up immediately; the first ride poll waits until the trip ETA. After `ready`, a near trip finishes in well under a minute. Prove that walk with mix **off** (`POST /admin/faults` `{"mode":"clear","mix":"off"}` on both sims) so it cannot flake; Settings defaults are the always-on 3% 5xx / 2% drop mix.
...
- `conservation` — `accepted = delivered + cancelled + failed + in_flight`. `in_flight` is the count of orders whose current state is not terminal (not derived from that equation). `parked` is orders with a parked work item and must be a subset of `in_flight` (park is not a lifecycle stage). `residual` is 0 when the books balance and parked ⊂ in-flight.
...
- `retry_rate` — fraction of attempts in the last 60s that are not the first attempt on that work item.
...
- `parked_list` — owner / reason / next action. Rendered read-only on `/` (Correctness). No Redrive.
- `sim_http` — per-sim `requests_per_min` and latency p50/p95 (restaurant = kitchen work, courier = dispatch/ride).
- `no_progress_beyond_threshold` — in-flight orders whose last applied event is older than 90s.

Calibrate reads `backlog`, `oldest_open`, and `http_429s` from this same GET. Do not add slot-use-vs-cap here (that waits for the outage slice).

### Dashboard

Vite SPA on **5173**. `/` is read-only metrics (cards, not charts). Three panes: **Business** (stages + terminal rates + e2e + oldest-age and stage), **Pipeline** (accept/reject, backlog by work type, retry rate, 429s, stretching ETAs, per-sim rate/latency), **Correctness** (lite fields + no-progress-beyond-threshold + parked list). Parked list is owner / reason / next action, read-only — no Redrive. `/control` load group: Calibrate · New cohort · Steady · Rush (optional **mult**) · Stop. Buttons POST through the same-origin `/loadgen` proxy. Header **Controls** opens `/control` in a new tab; the two routes do not share React state. Watch independently reads `/loadgen/status` on every poll, so a cohort minted in Control appears there without cross-tab state. Outage / crash / bonus buttons wait.
...
Scenario 2 keeps steady arrivals at **0.4×H**. On `/control`, click **Doom-confirm**, then **Restaurant blackout (60s)**; the latter posts exactly `{"mode":"blackout","seconds":60}` through `/rsim/admin/faults`. **Clear restaurant** posts `{"mode":"clear"}` to that same proxy. On `/`, Pipeline splits each sim's timeout/unknown responses from 5xx and 429, and shows active outbound leases against honest two-worker fleet caps: restaurant `used/16`, courier `used/16`, all tasks `used/48` (per-worker 8 / 8 / 24). Doom-confirm never goes through `/rsim`.
...
`mode` is `clear` (off), `5xx_before` (fail before the ledger write), `5xx_after` (write then 5xx), `drop` (apply the effect, then close without a body — timeline D), or **`blackout`** (timed; no success inside the 2s timeout — drop, classified unknown, same key). Blackout defaults off on **both** 8081 and 8082. Doom-confirm's targeted restaurant rule is separate from this mode and is listed as `confirm_unavailable` by `GET /admin/faults`; it remains active after a 60s blackout expires. `clear` removes both the global mode and targeted set. Crash will later use courier blackout as the park fixture. Direct curls remain valid; `/control` now owns only the scenario-2 restaurant outage buttons. No Kill, courier-blackout assist, or Redrive button lands in this slice.
...
Talk-track: watch the clean walk **first** (mix off). Then leave 3/2 on and point at duplicate attempts vs duplicate effects = 0. Duplicate attempts already includes extra cook polls on a clean walk (being prepared, then ready) — `> 0` is not a flakiness detector. Effects must stay 0. Duplicate attempts under mix is a rehearsal observation, not an automated pass condition.
...
- Worker `restart: "no"` on purpose: `restart: always` would reap the abandoned NULL attempt that scenario 3 needs to show. Two replicas via `deploy.replicas: 2` so H is measured at demo topology. Worker health stays inside the container (8083) — publishing it to the host would collide when both replicas start.
...
- Pipeline pane binds the existing snapshot keys in place and additively shows the outage split (`timeout` includes dropped/unknown responses, plus `http_5xx` and `http_429`) for each sim. `outbound_slots` derives live use from active DB leases and reports two-worker fleet caps 16 / 16 / 48 while retaining each per-worker cap. Oldest-age + stage live on Business. Parked list + no-progress live on Correctness. No worker-replica utilization pane and no Redrive.
