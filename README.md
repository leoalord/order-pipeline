...
Calibrate first so rush is sized to this machine; Normal can run on the fallback H, Rush cannot. Calibrate stops arrivals, waits for the current cohort to quiesce (cook/ride still occupy pans and bikes), then mints its own measurement cohort so leftover parked or stalled rows cannot pin `oldest_age_s` and walk the ramp to `h = 0`. It publishes H only after the measurement cohort also quiesces; a bounded start or finish timeout returns 504, restores the fallback H for Normal, and leaves Rush disabled. Recalibrate from the rail takes the same path.

1. **Calibrate** — measures H, the highest rate whose waiting backlog stays flat. Each step closes its arrival window and resolves its outstanding POSTs before it can be called sustainable. A step with either unresolved transport outcomes or non-429 HTTP failures is not sustainable. Status, the rail, and the calibrate payload separately report offered order intents, 201 accepted, door 429, other HTTP, and transport/timeout unknowns. If no sustainable H emerges after a few steps, the ramp aborts with a diagnostic and restores the fallback instead of walking to `calibrate_max_rps` or dead-ending Normal.
2. **New cohort** — isolates the demo walk from calibration traffic and resets the visible load counters.
3. **01 Normal** — steady `0.4×H`. Follow one ticket through every stage.
4. **02 Rush** — 60s at `1.5×H`, then drain back to `0.4×H`. Drain requires a recorded waiting peak above the pre-rush baseline, then sustained or baseline-level waiting recovery **and** a material net decline (or baseline return) in either total or parked-excluded oldest-age. Parking is shedding and is reported separately; it is not recovery. `POST /stop` waits for in-flight POSTs **and** cook/ride work to quiesce, and returns 504 if that wait times out. The rail's **Reset demo** uses `POST /stop?wait=false`: it stops arrivals and resolves POSTs without holding fault cleanup behind the full pipeline drain.
