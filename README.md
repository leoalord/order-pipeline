...
- `parked_list` — additive work-item id plus order / work type / owner / reason / next action.
  Rendered on `/` in the Workers drawer, where **Redrive** is the one write button.
...
Vite SPA on **5173**. One route: `/` is the whole presentation surface, and `/control` redirects there for old bookmarks.

The board renders the live lifecycle as stage columns of tickets between the **Restaurant** and **Delivery** zones, above a **Now showing** callout carrying that scenario's evidence, an evidence strip of cohort totals, three health chips (Restaurant · Workers · Delivery), and the correctness proof. The board follows one **focused order** from placement through delivery; clicking any ticket takes over the focus, and minting a new cohort clears it. Clicking a ticket, chip, or the proof opens a details drawer: the order trace (events, attempts, owners, idempotency keys, and the paste-an-ID lookup), zone dependency signals, correctness invariants, or the worker view with currently-leased and parked rows. Currently-leased rows carry order, work type, and worker owner, so the crash beat can record the exact order before the terminal kill.

**Redrive** is the one operator write. It sits on each parked row in the worker drawer and on the parked row under a focused order, and it is disabled while that row's dependency fault is still armed — redriving into an armed fault simply exhausts the budget and re-parks the same key.

Header **Presenter controls** slides the presenter rail in over the same page; it is the same React state, not a second tab. The rail holds the capacity baseline (**Calibrate**), courier fleet capacity (slider · Apply · batch Redrive of parked courier jobs), the numbered scenario sequence (01 Normal · 02 Rush · 03 Outage · 04 Worker crash · 05 Courier failure), **Setup & bonus beats** (New cohort · Cancel race · Fail void · Out of stock · Place burrito · Restore stock), and **Reset demo**. Buttons POST through the same-origin `/loadgen`, `/rsim`, or `/csim` proxies. There is no browser Kill button. Abort leftover `fail_void` with **Recover restaurant** (`{"mode":"clear"}`) — it sticks if you skip that.

The surface is sized for a shared screen, not a desk. No text on it is smaller than **11px**, stage names run 16–20px, and `tests/test_dashboard.py` fails the build if any rule drops below that floor — including the narrow-viewport overrides, which buy room from gaps and copy rather than from type. Verified with no clipping and no overflow at both **1280×720** and **1440×810**. Both side panels scroll as one body between a fixed heading and footer, so the five-scenario sequence is readable rather than crammed into an inner well; each is a `role="dialog"` with `aria-modal`, initial focus inside, a Tab trap, Escape to close, and focus returned to whatever opened it.

Two deliberate limits on what the chips claim. **Fault active** comes only from an armed simulator fault, never from error counts: the always-on mix keeps timeouts and 5xx permanently above zero, and a blackout drains those same counters by stopping traffic altogether. Raw per-lane counts stay visible in the zone drawer. And work that cannot progress reads as **Stalled work** on the Workers chip, attributed by work type — a parked courier dispatch stops its order without any worker being unhealthy.
...
```bash
# SPA (one route; Presenter controls opens the rail in place)
...
A quiet chips order with knobs off walks every assignment stage on `/`: placed → confirmed → being prepared → ready → out for delivery → delivered. Confirmed is queued (waiting for a pan); being prepared is cooking (on a pan) — the stage columns on the board. Watch that walk **first** (scenario 0), following the focused ticket. Stage columns bind to `snapshot.stages`; the evidence strip binds `conservation`, `terminal_rates_per_min`, `e2e_latency_s`, and `oldest_open`. The scenario callout's facts bind `accept_reject`, `backlog`, `retry_rate`, `http_429s`, `stretching_etas`, and `sim_http` for whichever beat is showing. The zone drawers bind `sim_http` and `outbound_slots`; the worker drawer binds `currently_leased_items`, `no_progress_beyond_threshold`, and `parked_list`; the correctness drawer binds conservation residual, duplicate attempts vs effects, startup scan, invalid transitions, orphaned tickets, and state-vs-last-event mismatch. The paste-an-ID trace lives in the order drawer. The parked row's additive work-item `id` is what its Redrive button posts. Point at attempts vs effects **second** — attempts may duplicate under the 3%/2% mix; effects must not. Rush (scenario 1): backlog and oldest-age rise, then backlog falls during drain. Oldest-age also falls unless the oldest order is intentionally parked; in that case the parked row supplies the owner and next action.
...
The same `Idempotency-Key` replays the first result (Stripe-style). Timeout retries must reuse that key — never mint a new one. `GET /admin/ledger` returns effect counts by key from the sim's SQLite ledger (not Postgres). Menu-item stock (`chips` / `taco` / `burrito`) is restaurant-only. `RSIM_STOCK_DEFAULT` boots each item at 10,000 so scenarios 0–3 cannot organically become stock rehearsals; the value is stored in the same SQLite file as effects and survives restaurant restarts. A cart with any unavailable item is a business 4xx: no ledger row, no decrement, whole order fails. A successful new accept inserts its effect and decrements every requested quantity in one SQLite transaction; the same stored confirm key replays without a second decrement. Courier has no `/admin/stock`. Fault `clear` never changes stock.
...
Rush assumes steady is already running and does **not** replay a minute of baseline. If calibrate 429s at the door first, raise `API_ACCEPT_CONCURRENCY` and recalibrate — kitchen/courier 3× is the intended busy beat. Presenter rail buttons POST the same paths through `/loadgen`; curls stay the Method spec and the fallback.

Boot ships a conservative fallback `LOADGEN_DEFAULT_H=0.25` so the walk works before any measurement. `GET /loadgen/status` reports `h_source` (`fallback` until a calibrate run) and `calibrated`; the rail shows **Fallback baseline · unverified** until then. Normal runs on the fallback, but **Rush stays disabled until Calibrate has measured this host** — rush peaks at `1.5 × H`, so on a host whose real H is higher the fallback peak lands below capacity and produces no backlog, no ETA stretch, and no 429s.
...
Scenario 2 keeps steady arrivals at **0.4×H**. In the presenter rail's **03 Outage** card, click **1 · Doom confirms**, then **2 · Blackout**; the latter posts exactly `{"mode":"blackout","seconds":60}` through `/rsim/admin/faults`. **Recover restaurant** posts `{"mode":"clear"}` to that same proxy. While the blackout is armed the Restaurant chip reads **Fault active** with its countdown, and the callout says orders hold until the lane recovers. Open the Restaurant zone drawer for the split: each sim's trailing-60-second timeout/unknown responses separated from 5xx and 429, plus active outbound leases against the configured two-worker fleet caps: restaurant `used/16`, courier `used/16`, all tasks `used/48` (per-worker 8 / 8 / 24). The used values are live leases; caps come from deployment configuration rather than live replica discovery. Doom-confirm never goes through `/rsim`.

Scenario 3 runs in two beats, in order. First, with steady arrivals still running and the always-on mix **on**, arm a catchable lease with **Arm visible lease** in the rail's **04 Worker crash** card; it posts the same `{"mode":"blackout","seconds":60}` to `/rsim/admin/faults`. A healthy confirm is milliseconds; blackout holds the outbound call for `sim_timeout_s + 0.5s` so the currently-leased rows in the Workers drawer stay readable. Open that drawer, then record an order id and its owner hostname. Match that hostname to the worker id from `docker compose ps -q worker`, stop that exact worker **from the Docker terminal** while the lease is in flight, and **Recover restaurant** immediately after the kill:

```bash
curl -sS -X POST http://localhost:8081/admin/faults \
  -H 'Content-Type: application/json' \
  -d '{"mode":"blackout","seconds":60}'
# Read the Workers drawer's currently-leased rows, then:
docker kill <full-worker-id-whose-prefix-matches-the-visible-owner>
curl -sS -X POST http://localhost:8081/admin/faults \
  -H 'Content-Type: application/json' \
  -d '{"mode":"clear"}'
# Read the recorded trace for the <=15s gap and survivor resume first, then:
...
Second, click **Blackout courier · 30s** in the rail's **05 Courier failure** card. A dispatch exhausts its budget of 5 and appears in the Workers drawer's parked list while its order remains `ready`. The Delivery chip reads **Fault active**, the Workers chip reads **Stalled work** naming the parked dispatch, and the courier-failure facts count parked courier work, dependency errors (timeout + 5xx), and courier busy 429s. Observe the row's owner, reason, and next action. Its **Redrive** button stays disabled — and says so — until the courier recovers. Press **Recover courier** (or wait for the timed blackout to expire), then press **Redrive** on that parked row:
...
The board—not a curl—is the runbook's operator path for `POST /work-items/{id}/redrive`. The item becomes immediately eligible with attempt count 0 and the same stored `(order_id, dispatch)` key, then the order delivers with one courier-ledger effect. Redriving before the blackout clears would simply exhaust and park the same job again, which is why every Redrive button is disabled while its dependency fault is armed.
...
Courier capacity starts at `CSIM_FLEET_SIZE=8` and can be changed live for presentation beats with `GET /admin/capacity` and `POST /admin/capacity` `{"fleet_size": 16}` on the courier sim. The presenter rail exposes the same setting as a slider bounded by the `min_fleet_size` / `max_fleet_size` the capacity endpoint reports. A change affects quotes for new dispatches immediately; existing ticket ETAs remain stable, and parked dispatch work still requires Redrive. Restarting the courier sim restores its configured boot fleet.
...
- One route, one React state: `/` polls `GET /snapshot` while the presenter rail POSTs `/loadgen/...`, `/rsim/...`, and `/csim/...` through the Vite proxy from the same page. A click changes the system; the board notices on the next poll. `/control` redirects to `/`.
