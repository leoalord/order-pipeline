...
`mode` is `clear` (off), `5xx_before` (fail before the ledger write), `5xx_after` (write then 5xx), `drop` (apply the effect, then close without a body — timeline D), or **`blackout`** (timed; no success inside the 2s timeout — drop, classified unknown, same key). Blackout is a fixture for later slices; default is off on **both** 8081 and 8082. Outage will POST restaurant blackout as the beat; crash will POST courier blackout as the park fixture. Direct curls; `/control` has load buttons only — outage/crash fault buttons wait.
...
- Compose carries wiring only (DSNs, hosts, ports, ledger paths, restaurant and courier base URLs, API sim admin URLs for `GET /snapshot`, loadgen API URL, dashboard Vite proxy targets). Knob defaults live in `APISettings` / `RSIMSettings` / `CSIMSettings` / `WorkerSettings` / `LoadgenSettings`. The dashboard's only `VITE_` knob is `VITE_API_BASE_URL`; compose leaves it unset and proxies instead.
...
- Pipeline pane binds the dinner-rush snapshot keys in place (accept/reject, backlog by work type, retry rate, 429s, stretching ETAs, per-sim HTTP). Oldest-age + stage live on Business. Parked list + no-progress live on Correctness. No worker-utilization pane and no slot-use-vs-cap cards — those wait for the outage slice. No Redrive.
